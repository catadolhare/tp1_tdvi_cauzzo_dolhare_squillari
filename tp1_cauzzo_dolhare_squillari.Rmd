...
```
```{r}
best_tree2 <- rpart(formula = satisfaction ~ Flight.Distance + Inflight.wifi.service + Ease.of.Online.booking + Food.and.drink + Online.boarding + Seat.comfort + Inflight.entertainment + On.board.service + Leg.room.service + Baggage.handling + Checkin.service + Inflight.service + Cleanliness, 
              data = entrenamiento, 
              method = "class",
...
reemplazar_na <- function(data, porcentaje_na) {
  for (i in 1:ncol(data)) {
    ...
    if (nuevos_na > 0) {
      # Escoger índices sin NA existentes para reemplazar
      na_indices <- sample(which(!is.na(data[[i]])), nuevos_na)
      data[na_indices, i] <- NA
    }
  }
  return(data)
}
...
```
```{r}
imputar_na <- function(data) {
  for (i in 1:ncol(data)) {
    if (is.numeric(data[[i]])) {
      # Imputar la media para columnas numéricas
      data[[i]][is.na(data[[i]])] <- mean(data[[i]], na.rm = TRUE)
    } else if (is.factor(data[[i]]) || is.character(data[[i]])) {
      # Imputar la moda para columnas categóricas
      moda <- names(sort(table(data[[i]]), decreasing = TRUE))[1]
...
entrenamiento_na_20_imputado <- imputar_na(entrenamiento_na_20)
validacion_na_20_imputado <- imputar_na(validacion_na_20)
testeo_na_20_imputado <- imputar_na(testeo_na_20)

best_na_20 <- buscar_hiperparametros(entrenamiento = entrenamiento_na_20_imputado, validacion = validacion_na_20_imputado, valores_maxdepth = valores_maxdepth2, valores_minsplit = valores_minsplit2, valores_minbucket = valores_minbucket2)
...
```{r}
#summary(entrenamiento_na_20) #7000NA
#summary(validacion_na_20)  #1500NA
summary(testeo_na_20)   #1500NA
